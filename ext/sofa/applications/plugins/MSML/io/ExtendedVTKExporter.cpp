...
******************************************************************************/
/*
 * ExtendedVTKExporter.cpp
 *
 *  Created on: 9 sept. 2009
 *      Author: froy
 */

#include "ExtendedVTKExporter.h"
#include "rmkdir.h"
...
SOFA_DECL_CLASS(ExtendedVTKExporter)

int ExtendedVTKExporterClass = core::RegisterObject("Export state vectors and meshes at each timestep")
.add< ExtendedVTKExporter >();

ExtendedVTKExporter::ExtendedVTKExporter()
: stepCounter(0), outfile(NULL)
, vtkFilename( initData(&vtkFilename, "filename", "output VTK file name"))
, fileFormat( initData(&fileFormat, (bool) false, "XMLformat", "Set to true to use XML format"))
, position( initData(&position, "position", "points position (will use points from topology or mechanical state if this is empty)"))
, writeEdges( initData(&writeEdges, (bool) false, "edges", "write edge topology"))
...
, writeQuadraticTetras( initData(&writeQuadraticTetras, (bool) false, "quadraticTetras", "write quadratic tetra topology"))
...
void ExtendedVTKExporter::init()
{
	...
	if (!topology)
	{
		serr << "ExtendedVTKExporter : error, no topology ." << sendl;
		return;
	}

	if (!mstate)
	{
		serr << "ExtendedVTKExporter : error, no mstate ." << sendl;
		return;
	}
...
void ExtendedVTKExporter::fetchDataFields(const helper::vector<std::string>& strData, helper::vector<std::string>& objects, helper::vector<std::string>& fields, helper::vector<std::string>& names)
{
	for (unsigned int i=0 ;i<strData.size() ; i++)
	{
		...
		{
			serr << "ExtendedVTKExporter : error while parsing dataField names" << sendl;
            continue;
		}
...
void ExtendedVTKExporter::writeData(const helper::vector<std::string>& objects, const helper::vector<std::string>& fields, const helper::vector<std::string>& names)
{
	sofa::core::objectmodel::BaseContext* context = this->getContext();
	...
	for (unsigned int i=0 ; i<objects.size() ;i++)
	{
		...
		if (!obj || !field)
		{
			serr << "ExtendedVTKExporter : error while fetching data field, check object name or field name " << sendl;
		}
		else
		{
...
void ExtendedVTKExporter::writeDataArray(const helper::vector<std::string>& objects, const helper::vector<std::string>& fields, const helper::vector<std::string>& names)
{
	sofa::core::objectmodel::BaseContext* context = this->getContext();
...
std::string ExtendedVTKExporter::segmentString(std::string str, unsigned int n)
{
...
void ExtendedVTKExporter::writeVTKSimple()
{
...
//	std::cout << filename << std::endl;
	...
	*outfile << "POINTS " << nbp << " float" << std::endl;
	...
	//testing
	int nPointsState = mstate->getSize();
	int nPointsTopology = topology->getNbPoints();
...
//		std::cout << topology->getPX(i) << " " << topology->getPY(i) << " " << topology->getPZ(i) << std::endl;
	...
	*outfile << std::endl;
	...
	//int number = (dynamic_cast<sofa::component::topology::QuadraticMeshTopology* > (topology))->getNbQuadTetrahedra();
	//Write Cells
	...
	numberOfCells = ( (writeEdges.getValue()) ? topology->getNbEdges() : 0 )
			       ...
			       +( (writeQuadraticTetras.getValue()) ? (dynamic_cast<sofa::component::topology::QuadraticMeshTopology* > (topology))->getNbQuadTetrahedra() : 0 )
	...
	totalSize =     ( (writeEdges.getValue()) ? 3 * topology->getNbEdges() : 0 )
				   ...
				   +( (writeQuadraticTetras.getValue()) ? 11 *(dynamic_cast<sofa::component::topology::QuadraticMeshTopology* > (topology))->getNbQuadTetrahedra() : 0 )
	...
	*outfile << "CELLS " << numberOfCells << " " << totalSize << std::endl;
	...
	if (writeQuadraticTetras.getValue())
	{
		for (int i=0 ; i<(dynamic_cast<sofa::component::topology::QuadraticMeshTopology* > (topology))->getNbQuadTetrahedra() ; i++)
			*outfile << 10 << " " <<  (dynamic_cast<sofa::component::topology::QuadraticMeshTopology* > (topology))->getQuadTetrahedra(i) << std::endl;
	}
...
void ExtendedVTKExporter::writeVTKXML()
{
	std::string filename = vtkFilename.getFullPath();
	std::ostringstream oss;
	oss << "_"<< nbFiles;
...
void ExtendedVTKExporter::writeParallelFile()
{
	std::string filename = vtkFilename.getFullPath();
...
//	std::cout << filename << std::endl;
	...
	*outfile << "    </PPoints>" << std::endl;
	...
	for(int i = 1; i < nbFiles; ++i)
	{
		std::ostringstream oss;
		oss << i;
		*outfile << "    <Piece Source=\"" << vtkFilename.getFullPath() << "_" << oss.str() << ".vtk" << "\"/>" << std::endl;
	}
...
void ExtendedVTKExporter::handleEvent(sofa::core::objectmodel::Event *event)
{
...
void ExtendedVTKExporter::cleanup()
{
...
void ExtendedVTKExporter::bwdInit()
{
