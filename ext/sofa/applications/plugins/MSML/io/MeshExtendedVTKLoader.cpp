...
#include "MeshExtendedVTKLoader.h"
...
SOFA_DECL_CLASS(MeshExtendedVTKLoader)

int MeshExtendedVTKLoaderClass = core::RegisterObject("Specific mesh loader for VTK file format.")
    .add< MeshExtendedVTKLoader >()
    ;

//Base VTK Loader
MeshExtendedVTKLoader::MeshExtendedVTKLoader() : MeshLoader()
		, reader(NULL)
{
	addAlias(&quadraticTetrahedra,"quadTetras");
	quadraticTetrahedra.setPersistent(false);
}

void MeshExtendedVTKLoader::addQuadraticTetrahedron(helper::vector<QuadraticTetrahedron>* pQuadraticTetrahedra, const QuadraticTetrahedron &p)
{
	pQuadraticTetrahedra->push_back(p);
}

void MeshExtendedVTKLoader::addQuadraticTetrahedron(helper::vector<QuadraticTetrahedron>* pQuadraticTetrahedra, unsigned int p0, unsigned int p1, unsigned int p2, unsigned int p3, unsigned int p4, unsigned int p5, unsigned int p6, unsigned int p7, unsigned int p8, unsigned int p9)
{
	addQuadraticTetrahedron(pQuadraticTetrahedra, QuadraticTetrahedron(p0, p1, p2, p3,p4,p5,p6,p7,p8,p9));
}

MeshExtendedVTKLoader::VTKFileType MeshExtendedVTKLoader::detectFileType(const char* filename)
{
    std::ifstream inVTKFile(filename, std::ifstream::in | std::ifstream::binary);

    if( !inVTKFile.is_open() )
        return MeshExtendedVTKLoader::NONE;
	...
	if (line.find("<?xml") != std::string::npos)
	{
		...
		if (line.find("<VTKFile") != std::string::npos)
			return MeshExtendedVTKLoader::XML;
		else
			return MeshExtendedVTKLoader::NONE;
	}
		...
		return MeshExtendedVTKLoader::LEGACY;
	else //default behavior if the first line is not correct ?
		return MeshExtendedVTKLoader::NONE;
}

bool MeshExtendedVTKLoader::load()
{
    ...
    MeshExtendedVTKLoader::VTKFileType type = detectFileType(filename);
    switch (type)
    {
		case XML:
...
bool MeshExtendedVTKLoader::setInputsMesh()
{
	...
	helper::vector<QuadraticTetrahedron >& my_QuadraticTetrahedra = *(quadraticTetrahedra.beginEdit());
	...
	else if (reader->inputCells && reader->inputCellTypes)
	{
		...
		for (int c = 0; c < nbf; ++c)
		{
			...
			case 12: // HEXAHEDRON
				...
				break;
			case 24: // QuadraticTetrahedron
				addQuadraticTetrahedron(&my_QuadraticTetrahedra, inFP[i+0], inFP[i+1], inFP[i+2], inFP[i+3],
											 inFP[i+4], inFP[i+5], inFP[i+6], inFP[i+7], inFP[i+8], inFP[i+9]);
				break;
			default:
	...
	tetrahedra.endEdit();
	hexahedra.endEdit();
	quadraticTetrahedra.endEdit();

	return true;
}

bool MeshExtendedVTKLoader::setInputsData()
{
...
//Legacy VTK Loader
bool MeshExtendedVTKLoader::LegacyVTKReader::readFile(const char* filename)
{
	std::ifstream inVTKFile(filename, std::ifstream::in | std::ifstream::binary);
	if( !inVTKFile.is_open() )
	{
...
bool MeshExtendedVTKLoader::XMLVTKReader::readFile(const char* filename)
{
...
MeshExtendedVTKLoader::BaseVTKReader::BaseVTKDataIO* MeshExtendedVTKLoader::XMLVTKReader::loadDataArray(TiXmlElement* dataArrayElement)
{
	return loadDataArray(dataArrayElement,0);
}

MeshExtendedVTKLoader::BaseVTKReader::BaseVTKDataIO* MeshExtendedVTKLoader::XMLVTKReader::loadDataArray(TiXmlElement* dataArrayElement, int size)
{
	return loadDataArray(dataArrayElement, size, "");
}

MeshExtendedVTKLoader::BaseVTKReader::BaseVTKDataIO* MeshExtendedVTKLoader::XMLVTKReader::loadDataArray(TiXmlElement* dataArrayElement, int size, std::string type)
{
	//Type
...
bool MeshExtendedVTKLoader::XMLVTKReader::loadUnstructuredGrid(TiXmlHandle datasetFormatHandle)
{
...
bool MeshExtendedVTKLoader::XMLVTKReader::loadPolydata(TiXmlHandle /* datasetFormatHandle */)
{
	serr << "Polydata dataset not implemented yet" << sendl;
	return false;
}

bool MeshExtendedVTKLoader::XMLVTKReader::loadRectilinearGrid(TiXmlHandle /* datasetFormatHandle */)
{
	serr << "RectilinearGrid dataset not implemented yet" << sendl;
	return false;
}

bool MeshExtendedVTKLoader::XMLVTKReader::loadStructuredGrid(TiXmlHandle /* datasetFormatHandle */)
{
	serr << "StructuredGrid dataset not implemented yet" << sendl;
	return false;
}

bool MeshExtendedVTKLoader::XMLVTKReader::loadStructuredPoints(TiXmlHandle /*datasetFormatHandle */)
{
	serr << "StructuredPoints dataset not implemented yet" << sendl;
	return false;
}

bool MeshExtendedVTKLoader::XMLVTKReader::loadImageData(TiXmlHandle /* datasetFormatHandle */)
{
