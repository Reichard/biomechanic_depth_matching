...
******************************************************************************/
#ifndef SOFA_COMPONENT_FORCEFIELD_LANDMARKSPRINGFORCEFIELD_INL
#define SOFA_COMPONENT_FORCEFIELD_LANDMARKSPRINGFORCEFIELD_INL
...
LandmarkSpringsForceField<DataTypes>::LandmarkSpringsForceField()
    ...
    , m_AdaptStiffness(initData(&m_AdaptStiffness,true,"adaptStiffness","adapt stiffness"))
    , m_ChargeFactor(initData(&m_ChargeFactor,1.0,"chargeFactor","charge factor"))
    , restMState(NULL)
//	, pp_0(NULL)
{    
...
template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::bwdInit()
{
    core::behavior::ForceField<DataTypes>::init();

    if (stiffness.getValue().empty())
    {
        std::cout << "LandmarkSpringsForceField : No stiffness is defined, assuming equal stiffness on each node, k = 100.0 " << std::endl;
    ...
    if (!restMState)
    {
        ...
        if (path.size() > 0)
        {
            serr << "LandmarkSpringsForceField : " << external_rest_shape.getValue() << " not found" << sendl;
        }
    }
    else
    {
        ...
        // sout << "LandmarkSpringsForceField : Mechanical state named " << restMState->getName() << " found for RestShapeSpringFF named " << this->getName() << sendl;
    }
...
template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::recomputeIndices()
{
    m_indices.clear();
    m_ext_indices.clear();
    ...
    if (m_indices.size()==0)
    {
        //	std::cout << "in LandmarkSpringsForceField no point are defined, default case: points = all points " << std::endl;
    ...
    {
        // std::cout << "in LandmarkSpringsForceField no external_points are defined, default case: points = all points " << std::endl;
...
template<class DataTypes>
const typename LandmarkSpringsForceField<DataTypes>::DataVecCoord* LandmarkSpringsForceField<DataTypes>::getExtPosition() const
{
    return (useRestMState ? restMState->read(core::VecCoordId::position()) : this->mstate->read(core::VecCoordId::restPosition()));
}

template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::addForce(const core::MechanicalParams* /* mparams */ /* PARAMS FIRST */, DataVecDeriv& f, const DataVecCoord& x, const DataVecDeriv& /* v */)
{
...
template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::addDForce(const core::MechanicalParams* mparams /* PARAMS FIRST */, DataVecDeriv& df, const DataVecDeriv& dx)
{
...
template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::draw(const core::visual::VisualParams * /* vparams */ )
{

}

template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::addKToMatrix(const core::MechanicalParams* mparams /* PARAMS FIRST */, const sofa::core::behavior::MultiMatrixAccessor* matrix )
{
//    //      remove to be able to build in parallel
//    // 	const VecIndex& indices = points.getValue();
//    // 	const VecReal& k = stiffness.getValue();
...
template<class DataTypes>
void LandmarkSpringsForceField<DataTypes>::addSubKToMatrix(const core::MechanicalParams* mparams /* PARAMS FIRST */, const sofa::core::behavior::MultiMatrixAccessor* matrix, const helper::vector<unsigned> & addSubIndex )
{
//    //      remove to be able to build in parallel
//    // 	const VecIndex& indices = points.getValue();
//    // 	const VecReal& k = stiffness.getValue();
...
#ifdef SOFA_HAVE_EIGEN2
template<class DataTypes>
const sofa::defaulttype::BaseMatrix* LandmarkSpringsForceField<DataTypes>::getStiffnessMatrix(const core::MechanicalParams* mparams) {
