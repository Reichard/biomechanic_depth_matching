...
******************************************************************************/
#include "ShapeMatchingNewmarkImplicitSolver.h"
...
ShapeMatchingNewmarkImplicitSolver::ShapeMatchingNewmarkImplicitSolver()
...
, m_LinearIterations( initData(&m_LinearIterations, 0, "linearIterations", "Number of linear iterations"))
...
void ShapeMatchingNewmarkImplicitSolver::solve(const core::ExecParams* params /* PARAMS FIRST */, double dt, sofa::core::MultiVecCoordId xResult, sofa::core::MultiVecDerivId vResult)
{
   ...
   // Reff = R + K0 * x0 - K  * un  + (M * ( Nma2 + Rayleighalpha*Nma4 ) + Rayleighbeta*K* Nma4) *vn + (M * ( Nma3+ Rayleighalpha*Nma5 )+ Rayleighbeta*K* Nma5)*an;
    ...
    //get both forcefields
	sofa::component::forcefield::LinearTetrahedralCorotationalFEMForceField<sofa::defaulttype::Vec3dTypes> *  fem;
	this->getContext()->get(fem);

	if(cpt<m_LinearIterations.getValue())
	{
		fem->setLinearMode(true);
	}
	else if(cpt<m_LinearIterations.getValue() != 0)
		fem->setLinearMode(false);

	sofa::component::forcefield::ShapeMatchingPoissonPotentialForcefield<sofa::defaulttype::Vec3dTypes> *  pot;
	this->getContext()->get(pot);

	sofa::component::mass::LinearMeshMatrixMass<sofa::defaulttype::Vec3dTypes,double> *  kinetic;
	this->getContext()->get(kinetic);

	sofa::core::MechanicalParams mparams(*params);

	double femEnergy, kineticEnergy, potEnergy;
	if(fem)
		femEnergy = fem->getPotentialEnergy(&mparams);
	else
		femEnergy = 0;

	if(kinetic)
		kineticEnergy = kinetic->getPotentialEnergy(&mparams);
	else
	{
		kineticEnergy = 0;
		std::cout<<"No mesh matrix mass found\n";
	}

	if(pot)
		potEnergy = pot->getPotentialEnergy(&mparams);
	else
		potEnergy = 0;
	...
	pot->setTotalElasticEnergy(femEnergy);
	pot->setTotalKineticEnergy(kineticEnergy);
	std::cout<<"FEM pot energy:"<<femEnergy <<"\n";
	std::cout<<"Kinetic pot energy:"<<kineticEnergy <<"\n";
	std::cout<<"Electric pot energy:"<<potEnergy <<"\n";


}
bool ShapeMatchingNewmarkImplicitSolver::printCurrentPositionAsMatlabFile(const core::ExecParams* params, const char* filename,int stepNumber)
{
...
bool ShapeMatchingNewmarkImplicitSolver::printVectorAsMatlabFiles(const core::ExecParams* params, core::MultiVecDerivId  vec,const char* filename, int stepNumber)
{
...
SOFA_DECL_CLASS(ShapeMatchingNewmarkImplicitSolver)

int ShapeMatchingNewmarkImplicitSolverClass = core::RegisterObject("Implicit time integratorusing Newmark scheme")
.add< ShapeMatchingNewmarkImplicitSolver >()
.addAlias("ShapeMatchingNewmark");
;
